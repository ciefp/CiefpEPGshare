#!/bin/bash
##setup command=wget -q "--no-check-certificate" https://raw.githubusercontent.com/ciefp/CiefpEPGshare/main/installer.sh -O - | /bin/sh
...
TMPPATH=/tmp/CiefpEPGshare

if [ ! -d /usr/lib64 ]; then
	PLUGINPATH=/usr/lib/enigma2/python/Plugins/Extensions/CiefpEPGshare
else
	PLUGINPATH=/usr/lib64/enigma2/python/Plugins/Extensions/CiefpEPGshare
fi
...
if [ -f /var/lib/dpkg/status ]; then
   ...
   wget https://github.com/ciefp/CiefpEPGshare/archive/refs/heads/main.tar.gz
   tar -xzf main.tar.gz
   cp -r 'CiefpEPGshare-main/usr' '/'
set +e
cd
sleep 2
...
echo ""
echo "#########################################################"
echo "#         CiefpEPGshare INSTALLED SUCCESSFULLY          #"
