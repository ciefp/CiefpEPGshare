...
try:
    ...
    logging.getLogger("CiefpEPGshare").warning("lxml module not found, attempting to install...")
    try:
        subprocess.run(["pip3", "install", "lxml"], check=True, capture_output=True, text=True)
        from lxml import etree
        LXML_AVAILABLE = True
        logging.getLogger("CiefpEPGshare").info("Successfully installed lxml")
    except Exception as e:
        logging.getLogger("CiefpEPGshare").error(f"Failed to install lxml: {str(e)}. Falling back to ElementTree")
        LXML_AVAILABLE = False
        import xml.etree.ElementTree as ET

PLUGIN_PATH = "/usr/lib/enigma2/python/Plugins/Extensions/CiefpEPGshare"
EPG_DIR = "/tmp/CiefpEPGshare"
...
IGNORE_LIST_FILE = os.path.join(EPG_DIR, "ignore_list.txt")
DEFAULT_IGNORED_FILES = {"epg_ripper_ALL_SOURCES1.xml.gz"}

# Configure logging for picons and critical errors only
logging.getLogger('').handlers = []
logging.getLogger("CiefpEPGshare").handlers = []
logging.basicConfig(
    level=logging.ERROR,
...
logger = logging.getLogger("CiefpEPGshare")
logger.debug("Initializing CiefpEPGshare logger")
...
def read_ignore_list():
    ...
    try:
        with open(IGNORE_LIST_FILE, 'r') as f:
            ...
            ignore_set.update(DEFAULT_IGNORED_FILES)
            return ignore_set
    except Exception as e:
        logger.error(f"Error reading ignore list: {str(e)}")
        return DEFAULT_IGNORED_FILES

def write_ignore_list(ignore_set):
    try:
        ...
        logger.error(f"Error writing ignore list: {str(e)}")

class XMLSelectionScreen(Screen):
    skin = """
        <screen name="XMLSelectionScreen" position="center,center" size="1280,852" title="..:: CiefpEPGshare - Select XML File ::..">
            <widget name="xmlList" position="0,0" size="880,720" scrollbarMode="showAlways" itemHeight="40" font="Regular;32" />
            <widget name="sideLogo" position="880,0" size="400,720" alphatest="on" />
            <widget name="bottomLogo" position="0,720" size="1280,132" alphatest="on" />
        </screen>
    """

    def __init__(self, session):
        Screen.__init__(self, session)
        self["xmlList"] = MenuList([], enableWrapAround=True)
        self["sideLogo"] = Pixmap()
        self["bottomLogo"] = Pixmap()
        ...
        self.xml_files = []
        self.onLayoutFinish.append(self.ensureIgnoreList)
        self.onLayoutFinish.append(self.fetchXMLList)
        self.onLayoutFinish.append(self.loadSideLogo)
        self.onLayoutFinish.append(self.loadBottomLogo)

    def ensureIgnoreList(self):
        # Ensure default ignored files are in the ignore list
        ignore_set = read_ignore_list()
        if DEFAULT_IGNORED_FILES.issubset(ignore_set):
            return
        ignore_set.update(DEFAULT_IGNORED_FILES)
        write_ignore_list(ignore_set)
        logger.info("Added default ignored files to ignore_list.txt")

    def fetchXMLList(self):
        try:
            headers = {
            ...
            # Extract .xml.gz files using regex
            xml_files = re.findall(r'href="([^"]*\.xml\.gz)"', html_content)
            ignore_set = read_ignore_list()
            self.xml_files = [f for f in xml_files if f.endswith('.xml.gz') and os.path.basename(f) not in ignore_set]
            if not self.xml_files:
                self["xmlList"].setList(["No XML files found or all are ignored"])
                logger.error("No XML files found or all are ignored at %s", BASE_URL)
                return
            self["xmlList"].setList([os.path.basename(f) for f in self.xml_files])
        except requests.exceptions.RequestException as e:
            logger.error(f"Error fetching XML list: {str(e)}")
            self["xmlList"].setList([f"Error: {str(e)}"])
        except Exception as e:
            logger.error(f"General error fetching XML list: {str(e)}")
            self["xmlList"].setList([f"Error: {str(e)}"])

    def selectXML(self):
        current = self["xmlList"].getCurrent()
        if current and not current.startswith("Error:"):
            xml_url = f"{BASE_URL}/{current}"
            self.session.open(CiefpEPGshare, xml_url)
        else:
    ...
    def ignoreXML(self):
        ...
        if current and not current.startswith("Error:"):
            ignore_set = read_ignore_list()
            ignore_set.add(current)
            write_ignore_list(ignore_set)
            self.fetchXMLList()  # Refresh the list to exclude the ignored file
            logger.info(f"XML file {current} added to ignore list")
        else:
            logger.error("No valid XML file selected for ignoring")
    ...
    def loadSideLogo(self):
        logo_path = os.path.join(PLUGIN_PATH, "side_logo.png")
        if os.path.exists(logo_path):
            try:
                pixmap = LoadPixmap(logo_path)
                if pixmap and self["sideLogo"].instance:
                    self["sideLogo"].instance.setPixmap(pixmap)
            except Exception as e:
                logger.error(f"Error loading side logo: {str(e)}")
        else:
            logger.error(f"Side logo not found: {logo_path}")

    def loadBottomLogo(self):
        logo_path = os.path.join(PLUGIN_PATH, "bottom_logo.png")
        if os.path.exists(logo_path):
            try:
                pixmap = LoadPixmap(logo_path)
                if pixmap and self["bottomLogo"].instance:
                    self["bottomLogo"].instance.setPixmap(pixmap)
            except Exception as e:
                logger.error(f"Error loading bottom logo: {str(e)}")
        else:
            logger.error(f"Bottom logo not found: {logo_path}")

class CiefpEPGshare(Screen):
    skin = """
        <screen name="CiefpEPGshare" position="center,center" size="1800,800" title="..:: CiefpEPGshare v1.0 ::..">
    ...
    def __init__(self, session, epg_url):
        Screen.__init__(self, session)
    ...
    def downloadAndParseData(self):
        cache_file = os.path.join(EPG_DIR, os.path.basename(self.epg_url).replace('.gz', ''))
        ...
        try:
            ...
            response = requests.get(self.epg_url, headers=headers, timeout=30)
            response.raise_for_status()
...
def main(session, **kwargs):
    session.open(XMLSelectionScreen)

def Plugins(**kwargs):
    return [PluginDescriptor(
        name="CiefpEPGshare",
        description="EPGshare EPG plugin v1.0",
